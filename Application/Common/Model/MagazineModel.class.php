...
class MagazineModel extends Model {
    ...
    public function getMagazine($data,$page,$pageSize=10) {
        $conditions = $data;
        ...
        $list = $this->_db->where($conditions)
            ->order('listorder desc ,magazine_id desc')
    ...
    public function getMagazineCount($data = array()){
        $conditions = $data;
        if(isset($data['title']) && $data['title']) {
            $conditions['title'] = array('like','%'.$data['title'].'%');
        }
    ...
    public function find($id) {
        $data = $this->_db->where('magazine_id='.$id)->find();
        return $data;
    }
    ...
    public function updateMagazineListorderById($id, $listorder) {
        if(!$id || !is_numeric($id)) {
            throw_exception('ID不合法');
        }
    ...
    public function getMagazineByMagazineIdIn($magazineIds) {
        ...
        $data = array(
            ...
            'magazine_id' => array('in',implode(',', $magazineIds)),
        );
    ...
    public function getRank($data = array(), $limit = 100) {
        $list = $this->_db->where($data)->order('count desc,magazine_id desc ')->limit($limit)->select();
        return $list;
    }
